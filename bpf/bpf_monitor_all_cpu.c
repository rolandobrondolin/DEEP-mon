...
int trace_switch(struct sched_switch_args *ctx) {
        ...
        for(i = 0; i < NUM_CPUS; i++) {
                ...
                if(topology_info.ht_id > NUM_CPUS) {
                        send_error(ctx, 2);
                        return 0;
                } else if (topology_info.sibling_id < i) {
                  // already processed, we can break
                  break;
                }

                //retrieve pid of running task
                int old_pid;
                if (processor_id == bpf_get_smp_processor_id()) {
                        old_pid = ctx->prev_pid;
                } else {
                        old_pid = topology_info.running_pid;
                }
                ...
                if(status_old.pid == old_pid) {
                        ...
                        bpf_probe_read(&sibling_info, sizeof(sibling_info), processors.lookup(&sibling_id));
                        if(sibling_info.ht_id != sibling_id) {
                        ...
                        //We got the information we needed, now update the processors fields
        ...
        //         // here just update the selector and reset counter if needed
        //         u64 last_ts_pid_in = 0;
        //         if(status_new.bpf_selector) {
        //                 last_ts_pid_in = status_new.ts[1];
        ...
        //         }
        //
        //         if(status_new.bpf_selector != bpf_selector || last_ts_pid_in + STEP < ts) {
        ...
        //                 }
        //                 if(new_pid == 0) {
        //                         idles.update(&processor_id, &status_new);
        //                 } else {
